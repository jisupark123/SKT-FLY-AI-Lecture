...
def translate(img, pt):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            x, y = np.subtract((j, i), pt)  # 좌표 이동
            if contain((y, x), img.shape):  # 좌표가 영상 내부이면
                dst[i, j] = img[y, x]  # 좌표 이동

    return dst


image = cv2.imread("week4/Chap08/images/translate.jpg", cv2.IMREAD_GRAYSCALE)
if image is None:
