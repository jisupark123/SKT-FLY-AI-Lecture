...
h, w = image1.shape[:2]  # 영상의 높이, 너비 구하기
cx, cy = w // 2, h // 2  # 영상의 중심 좌표(cx, cy)
cv2.circle(image1, (cx, cy), 100, 255, -1)  # 중심에 원 그리기
cv2.rectangle(image2, (0, 0, cx, h), 255, -1)  # 왼쪽 영역에 사각형 그리기
...
image6 = cv2.bitwise_not(image1)  # 논리 부정 (행렬 반전)
